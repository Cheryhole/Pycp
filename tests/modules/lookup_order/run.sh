...
# 模块查找顺序测试
# ---------------------------------------------------------------------
# 在沙箱（build/lookup_order_work，dist 的一份副本）中验证：
#   1) exe 目录 stdlib/ 下的 .pycp 源码模块可被导入（第 2 层）
#   2) 同名模块同时在 stdlib/ 与 cwd 时，stdlib（第 2 层）优先
#   3) 同一目录下同时有 .so 与 .pycp 时，动态库优先
#
...
run_case "第 2 层（stdlib）优先于第 3 层（cwd）" \
	"LOOKUP PASS: stdlib over cwd" "shadow_main.pycp"

# --- 用例 3：同层同名 .so 与 .pycp，验证动态库优先 ---
...
run_case "同层动态库优先于 .pycp 源码" \
	"LOOKUP PASS: so over pycp" "probe_main.pycp"
